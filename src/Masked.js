...
class Masked extends React.Component {
    ...
    constructor(props){
        super(props);
        this.state = {playingWord : randomWord,usedLetters : [], winningLettersArray:[], key : 23, showButton : false};
        this.getClickedLetterFromBoard = this.getClickedLetterFromBoard.bind(this);
        this.playing = this.playing.bind(this);
     
    }

    //méthode permettant de masquer le mot de la partie et de faire apparaitre une lettre dés qu'elle est découverte
    ...
    //méthode permettant de récupérer les lettres cliquées par l'utilisateur et de les stocker dans un tableau

    getClickedLetterFromBoard(clickedLetterFromBoard){
        
        this.setState(previousState => ({
            usedLetters: [...previousState.usedLetters, clickedLetterFromBoard]}));  
    
    }

    //méthode permettant de faire glisser les lettres trouvées dans un tableau comprenant les lettres faisant partie du mot

    playing(phrase,arrayLetters,winningLettersArray){
        var phraseArray = phrase.split('');
        var lastClickedLetter = arrayLetters[arrayLetters.length - 1];
 

         for(var i=0; i<phraseArray.length; i++) {
            // Si c'est le cas :
            if(phraseArray[i]===lastClickedLetter) {
                winningLettersArray.push(lastClickedLetter)
            }
    ...
    componentDidUpdate() {
        ...
        let phraseArray = this.state.playingWord.split('');
        let winningLettersArray = this.state.winningLettersArray;
    ...
    render(){
         ...
         return (
             ...
             <div className='content'>
                ...
                <div className='game'>{this.computeDisplay(this.state.playingWord,this.state.usedLetters)}</div>
                <Board className='board' getLetter={this.getClickedLetterFromBoard}></Board>
                <div>{this.playing(this.state.playingWord,this.state.usedLetters,this.state.winningLettersArray)}</div> 
                { this.state.showButton ? <Button getResetAction={this.reset} /> : null }         
             </div> 
         )        
    }
